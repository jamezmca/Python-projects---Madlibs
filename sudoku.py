def find_next_empty(puzzle):
    #finds the next row, col on the puzzle that's not fileld yet __> rep with -1
...
def solve_sudoku(puzzle):
    ...
    #step 1: where on the puzzle can i go? choose a spot to guess
    row, col = find_next_empty(puzzle)

    # step 1.1: if there is nowhere left, then were done because we only allowed valid inputs
    if row is None:
        return True

    # step 2: if there is a plce to put a number, then make a guess between 1 and 9
    for guess in range(1,10): # range(1,10) is 1, 2, 3, .. 9
        ...
        if is_valid(puzzle, guess, row, col):
            ...
            # step 4: recursively call our function to the enddddd
