...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # cast it to an integer, and if it's not, then we say 
            # its invalid; if that spot is not available on the board we also
            # say it's invalid
            try: 
                val = int(square)
...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            ...
            # get the square based of minimax algoritihm
            square = self.minimax(game, self.letter)['position']
        return square

    def minimax(self, state, player):
        max_player = self.letter #yourself we wanna win
        ...
        if player == max_player:
            best = {'position': None, 'score': -math.inf} # each score should maximise (be larger than)
        else:
